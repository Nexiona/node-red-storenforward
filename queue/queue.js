...
module.exports = function (RED) {
  ...
  function QueueNode(config) {
    ...
    var dbFilename = this.inMemoryDb ? ":memory:" : this.sqlite;
    if (
      dbFilename === ":memory:" &&
      (dbFilename === undefined || dbFilename == "")
    ) {
      node.error(
        "In-memory DB selected but no filename specified. Falling back to default filename."
      );
      dbFilename = "defaultQueueDb.sqlite"; // Fallback filename if none specified
    }
    /**
     * Queue
     * @type {PersistentQueue}
     */
    var queue = new Queue(dbFilename);
    ...
    function statusOutput() {
      ...
      if (queue.isOpen() && !queue.isEmpty() && isConnected) {
        s = {
          fill: "green",
          shape: "ring",
          text: "Processing" + remaining,
        };
      } else if (queue.isOpen() && queue.isEmpty() && isConnected) {
        s = {
          fill: "green",
          shape: "dot",
